...
export function parseGrid(text: string): string[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.length !== width) {
      throw new Error(
        `File is not rectangular: line ${i + 1} has width ${line.length}, expected ${width}`,
      );
    }
    checkPrintableAscii(line, i + 1);
  }
  return lines;
}
...
export function readSlice(lines: string[], rect: Rect): string {
  ...
  for (let r = rect.r1; r <= rect.r2; r++) {
    out.push(lines[r - 1]!.slice(rect.c1 - 1, rect.c2));
  }
  return out.join("\n");
}
...
export function writeSlice(
  ...
  if (fill !== undefined) {
    ...
    const c = fill.charCodeAt(0);
    if (c < 0x20 || c > 0x7e)
      throw new Error(`--fill must be a printable ASCII character`);
  }
  ...
  if (fill === undefined) {
    ...
    for (let i = 0; i < contentLines.length; i++) {
      if (contentLines[i]!.length < w) {
        contentLines[i] =
          contentLines[i]! + fill.repeat(w - contentLines[i]!.length);
      }
    }
  }
...
function usage(): never {
  console.error(
    [
      "Usage:",
      "  text-grid <file> read <range>",
      "  text-grid <file> write <range> <content> [--fill CHAR]",
      "",
      "Range formats:",
      ...
      "  Content may be piped via stdin (omit the positional arg).",
    ].join("\n"),
  );
  exit(1);
...
async function main() {
  ...
  if (command === "read") {
    ...
    process.stdout.write(readSlice(lines, rect) + "\n");
    return;
  }
