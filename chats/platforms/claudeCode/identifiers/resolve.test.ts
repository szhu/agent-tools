...
describe("resolveAddress", () => {
  test("all projects when pathInput is /", async () => {
    const result = await resolveAddress({ pathInput: "/" }, "/any/cwd", context);
  ...
  test("project by pathInput '.' uses cwd", async () => {
    const result = await resolveAddress({ pathInput: "." }, alphaProjectPath, context);
    expect(result).toEqual({
      type: "project",
      projectPath: alphaProjectPath,
      projectDir: alphaDir,
  ...
  test("throws for unknown project", async () => {
    await expect(
      resolveAddress({ pathInput: "/Users/alice/Code/unknown" }, "/any/cwd", context),
